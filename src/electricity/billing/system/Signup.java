...
public class Signup extends JFrame implements ActionListener{
    ...
    JButton create, back;
    Choice accountType;
    JTextField meter, username, name, password, id;
    Signup(){
        ...
        empId.setBounds(100, 90, 140, 20);
        empId.setForeground(Color.GRAY);
        empId.setFont(new Font("Tahoma", Font.BOLD, 14));
        ...
        id.addFocusListener(new FocusListener() {
            ...
            public void focusLost(FocusEvent fe) {
                try {
                    Conn c  = new Conn();
                    ResultSet rs = c.s.executeQuery("select * from login where meter_no = '"+id.getText()+"'");
                    while(rs.next()) {
                        name.setText(rs.getString("name"));
                    }
                } catch (Exception e) {
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == create) {
            ...
            String sempId = id.getText();
            
            
            // Validate email(username) format
            String emailRegex = "^[A-Za-z0-9+_.-]+@(.+)$";
            if (!susername.matches(emailRegex)) {
            ...
            if (susername.isEmpty()) {
                JOptionPane.showMessageDialog(null, "Username (email) cannot be empty.");
                return;
            }
            ...
            try {
                ...
                }else if(atype.equals("Employee")){
                     query = "update login set username = '"+susername+"', password = '"+spassword+"', user = '"+atype+"' where meter_no = '"+sempId+"'";
                }else {
